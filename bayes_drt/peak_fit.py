...
def HN_distribution(tau,t0,alpha,beta):
	...
	g = (1/np.pi)*(tau/t0)**(beta*alpha)*np.sin(alpha*theta)/(1+2*np.cos(np.pi*beta)*(tau/t0)**beta + (tau/t0)**(2*beta))**(alpha/2)
	return g
...
def fit_data(x0,freq,Z,R_inf=0,inductance=0,weights=None,lambda_x=10):
	...
	result = least_squares(resid,x0,args=(flat_weights,lambda_x),bounds=(lb,ub))#=([0,-np.inf,0,0]*len(peaks),[np.inf,np.inf,1,1]*len(peaks)))
