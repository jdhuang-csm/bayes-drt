...
exp_files = [lib_files; tco_files];
suffixes = {'LIB_data','LIB_data_qtr','PDAC'};

% DRT only
fit_files(exp_files,@jh_DRT_TpDDT_BpDDT,'Exp_DRT_modality.csv');


function fit_files(files,fun,modality_file,varargin)

    parser = inputParser();
    addOptional(parser,'suffixes',{})
    ...
    for n = 1:length(files)
        file = files(n);
        disp(file.name)
        [modality,betak,Rml,muml,wml,tl,Fl,Z_res] = jh_fit_sim_file(file.name,fun,file.folder,false);
        modalities(n,:) = modality;
        
        if ~isempty(parser.Results.suffixes)
            suffix = strcat(parser.Results.suffixes{n},'.csv');
        else
